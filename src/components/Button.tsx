...
const getVariant = (type: ButtonType) => {
  ...
  const hoverState = css<Props>`
    background-color: ${(p) => {
      switch (type) {
        case "normal":
          return getColor(p, p.color, ["darken", "0.8"]);
        default:
          return getColor(p, p.color);
      }
    }};
    border-color: ${(p) => {
      switch (type) {
  ...
  return css<Props>`
    background-color: ${(p) => {
      switch (type) {
        case "fade":
          return getColor(p, p.color, ["alpha", "0.4"]);
        case "ghost":
          return getColor(p, p.color, ["alpha", "0"]);
        case "clear":
          return getColor(p, p.color, ["alpha", "0"]);
        default:
          return getColor(p, p.color);
    ...
    border: ${(p) => p.theme.button.border}px solid
      ${(p) => {
        switch (type) {
          case "fade":
            return getColor(p, p.color, ["alpha", "0"]);
    ...
    small {
      color: ${(p) => {
        switch (type) {
          case "normal":
            return getColor(p, p.color, "defaultTheme");
          default:
            return getColor(p, p.color);
        }
      }} !important;
...
export const Button = styled.button<Props>`
  ...
  padding: ${(p) => {
    ...
    switch (p.size) {
      case "sm":
        return `${getSpace(p, "xs")} ${getSpace(p, "sm")}`;
      case "lg":
        return `${getSpace(p, "md")} ${getSpace(p, "lg")}`;
      default:
        return `${getSpace(p, "sm")} ${getSpace(p, "md")}`;
    }
  }};
  border-radius: ${({ theme, capsule }) =>
    capsule ? "9999px" : `${theme.button.corner}px`};
  cursor: pointer;
  outline: none;
